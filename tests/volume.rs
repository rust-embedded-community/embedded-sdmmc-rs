...
use embedded_sdmmc::blocking::{Error, Mode, VolumeIdx, VolumeManager};
...
fn open_all_volumes() {
    ...
    assert!(matches!(
        volume_mgr.open_raw_volume(VolumeIdx(0)),
    ...
    assert!(matches!(
        volume_mgr.open_raw_volume(VolumeIdx(1)),
        Err(Error::VolumeAlreadyOpen)
    ));
    ...
    assert!(matches!(
        volume_mgr.open_raw_volume(VolumeIdx(2)),
        Err(Error::FormatError(_e))
    ));

    // This isn't a valid volume
    assert!(matches!(
        volume_mgr.open_raw_volume(VolumeIdx(3)),
        Err(Error::FormatError(_e))
    ));

    // This isn't a valid volume
    assert!(matches!(
        volume_mgr.open_raw_volume(VolumeIdx(9)),
...
fn close_volume_too_early() {
    ...
    let _test_file = volume_mgr
        .open_file_in_dir(root_dir, "64MB.DAT", Mode::ReadOnly)
